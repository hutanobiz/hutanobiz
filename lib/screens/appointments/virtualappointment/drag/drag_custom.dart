...
class Draggable<T> extends StatefulWidget {
  ...
  /// recognizing a drag.
  @protected
  Future<MultiDragGestureRecognizer> createRecognizer(
      GestureMultiDragStartCallback onStart) async {
    switch (affinity) {
...
class LongPressDraggable<T> extends Draggable<T> {
  ...
  @override
  Future<DelayedMultiDragGestureRecognizer> createRecognizer(
      GestureMultiDragStartCallback onStart) async {
...
class _DraggableState<T> extends State<Draggable<T>> {
  ...
  void initState() {
    super.initState();
    widget.createRecognizer(_startDrag).then((value) {
      _recognizer = value;
    });
  }
